...
const SKIP_FIELDS = new Set([
  'raw_payload', 'updated_at', 'created_at', 'deleted_at',
  'user_agent', 'url', 'remember_token', 'password',
  'instrument_id', 'user_id', 'approved_by', 'procedure_schema_id',
  'id', 'pdf_hash', 'pdf_path', 'pdf_size',
]);
...
function formatDetails(log: AuditLogEntry): string {
  ...
  if (log.event === 'created' && log.new_values) {
    if (entity === 'CalibrationSession') {
      ...
      if (v.ambient_temperature)  parts.push(`Temperatura: ${v.ambient_temperature} °C`);
      if (v.ambient_humidity)     parts.push(`Humedad: ${v.ambient_humidity} %`);
      if (v.calibration_date)     parts.push(`Fecha: ${formatValue('calibration_date', v.calibration_date)}`);
      return parts.join(' · ');
    }
    if (entity === 'User') {
      const v = log.new_values;
  ...
  if (log.event === 'updated' && log.new_values) {
    const changed = Object.entries(log.new_values)
      ...
      .map(([k, v]) => {
        ...
        if (oldVal !== undefined && oldVal !== null && String(oldVal) !== String(v)) {
...
export default function AuditLogPage() {
  ...
  const filtered = logs.filter(l =>
    ...
    (EVENT_LABELS[l.event] || '').toLowerCase().includes(search.toLowerCase()) ||
          ...
          <tbody>
            {loading ? (
              Array.from({ length: 3 }).map((_, i) => (
                <tr key={i} className="td-theme">
                  <td colSpan={5} className="px-4 py-3">
                    <div className="h-4 rounded animate-pulse" style={{ backgroundColor: 'var(--bg-hover)' }} />
                  </td>
                </tr>
              ))
            ) : filtered.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-4 py-8 text-center text-[11px]" style={{ color: 'var(--text-muted)' }}>
                  Sin registros
                </td>
              </tr>
            ) : (
              filtered.map((log, i) => {
                ...
                const eventStyle = EVENT_COLORS[log.event] ?? { bg: 'rgba(255,165,38,0.1)', color: '#FFA526' };
                ...
                return (
                  <tr key={i} className="td-theme hover-bg transition-colors">
                    ...
                    <td className="px-4 py-2.5 whitespace-nowrap" style={{ color: 'var(--text-muted)' }}>
                      {entityLabel} {log.auditable_id ? `#${log.auditable_id}` : ''}
                    </td>
                    <td className="px-4 py-2.5 hidden sm:table-cell text-[10px] max-w-[280px] truncate"
