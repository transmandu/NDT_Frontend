...
export default function DashboardPage() {
  const { data: sessions = [] } = useQuery<CalibrationSession[]>({
    ...
    queryFn: () =>
      api.get("/calibration/sessions").then((r) => r.data.data || []),
  });
  ...
  // Calcula el lunes de la semana actual para que la gráfica siempre empiece en lunes
  ...
  const activityData = weekDays.map((dateStr) => {
    const daySessions = sessions.filter((s) =>
      s.created_at.startsWith(dateStr),
    );
    const dayName = new Date(dateStr + "T12:00:00").toLocaleDateString("es", {
      weekday: "short",
    });
    return {
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="lg:col-span-1 panel rounded-md shadow-sm overflow-hidden flex flex-col h-[280px]">
          ...
          <div className="flex-1 p-4 flex flex-col justify-center">
            {/* Numeric height on ResponsiveContainer avoids the width/height=-1 race
                condition that occurs when height="100%" is used and the parent's
                computed size isn't available yet on first render. */}
      ...
      <div className="w-full lg:w-[65%]">
        <div
          ...
          className="panel rounded-md shadow-sm overflow-hidden flex flex-col min-h-70 w-full"
        >
          <div
            className="px-4 py-3 flex justify-between items-center bg-(--bg-panel) shrink-0"
              ...
              <tbody>
                {recentSessions.length === 0 ? (
                  <tr>
                    <td
                      colSpan={5}
                      className="px-4 py-12 text-center text-[11px]"
                      style={{ color: "var(--text-muted)" }}
                    >
                  ...
                  recentSessions.map((s) => (
                    <ActivityRow
                      ...
                      date={new Date(s.created_at).toLocaleDateString("es", {
                        day: "2-digit",
                        month: "short",
                        year: "numeric",
                      })}
                      status={
                        s.status === "approved"
                          ...
                          : s.status === "pending_review"
                            ? "Auditoría Pendiente"
                            : s.status === "rejected"
                              ? "Devuelto p/ Corrección"
                              : "Toma de Datos (Borrador)"
                      }
                      color={
                        s.status === "approved"
                          ? COLORS.success
...
function CompactMetric({
  ...
  return (
    <div
      className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border shadow-sm transition-colors cursor-default"
      style={{
        backgroundColor: "var(--bg-panel)",
        borderColor: "var(--border-color)",
      }}
...
function ActivityRow({
  ...
  return (
    <tr className="td-theme hover-bg transition-colors border-b last:border-0 border-(--border-color)">
      <td className="px-4 py-3 font-mono text-[11px] whitespace-nowrap text-blue-600 dark:text-blue-400 font-medium">
        {id}
      </td>
      <td className="px-4 py-3 font-medium whitespace-nowrap truncate max-w-50 text-[11px] text-(--text-main)">
        {inst}
      </td>
      <td
        ...
        <div className="relative inline-block text-left" ref={menuRef}>
          ...
          <AnimatePresence>
            {isOpen && (
              ...
              >
                <p
                  className="px-3 py-1.5 text-[9px] uppercase font-semibold border-b border-gray-100 dark:border-gray-800"
                  style={{ color: "var(--text-muted)" }}
                >
                  Opciones de Trámite
                </p>
                <Link
                  href={`/calibration?review=${rawId}`}
                  className="px-3 py-2 text-xs text-left hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
                  style={{ color: "var(--text-main)" }}
                >
