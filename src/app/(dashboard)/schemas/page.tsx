...
function buildColumns(
  ...
  return [
    ...
    },
    {
      ...
      enableColumnFilter: true,
      accessorFn: (row) =>
        supportedCodes.includes(row.code) ? "Implementado" : "Sin estrategia",
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-1.5">
            ...
            <span
              ...
              style={{ color: has ? SUCCESS : WARNING }}
            >
  ...
  const content = (
    ...
    >
      ...
      >
        ...
        <p
          className="text-[10px] font-mono font-bold mb-3"
          style={{ color: ACCENT }}
        >
          {schema.code}
        </p>
        <p className="text-[11px] mb-1" style={{ color: "var(--text-muted)" }}>
          El esquema se marcará como <strong>inactivo</strong>. No podrá usarse
          en nuevas calibraciones.
        </p>
        <p className="text-[10px] mb-6" style={{ color: "var(--text-muted)" }}>
          Las <strong>{schema.calibration_sessions_count ?? 0}</strong> sesiones
          históricas asociadas permanecerán intactas.
        </p>
...
export default function SchemasPage() {
  const [deactivateTarget, setDeactivateTarget] =
    useState<ProcedureSchema | null>(null);
  ...
  // 1. SOLUCIÓN AL RENDERIZADO INFINITO DE ESLINT (Envolver en useMemo)
  ...
  const stats = useMemo(
    () => ({
      ...
      sessions: schemas.reduce(
        (acc: number, s) => acc + (s.calibration_sessions_count ?? 0),
        0,
      ),
    }),
  ...
  const deactivateMut = useMutation({
    ...
    onError: (err: unknown) => {
      const msg = isAxiosError(err)
        ? (err.response?.data?.message ?? "Error al desactivar el esquema.")
        : "Error al desactivar el esquema.";
      toast.error(msg);
      setDeactivateTarget(null);
    },
  });
  ...
  const activateMut = useMutation({
    mutationFn: (id: number) =>
      api.put(`/calibration/schemas/${id}`, { is_active: true }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["schemas"] });
    ...
    onError: (err: unknown) => {
      const msg = isAxiosError(err)
        ? (err.response?.data?.message ?? "Error al activar el esquema.")
        : "Error al activar el esquema.";
      toast.error(msg);
    },
  });

  /* ─── Columns ── */
  const columns: ColumnDef<ProcedureSchema>[] = useMemo(
    () =>
      buildColumns(
        supportedCodes,
    ...
    <div className="space-y-4 w-full animate-fadeIn">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          {
            label: "Con Motor Matemático",
            value: stats.withEngine,
            color: C.accent,
            icon: Cpu,
          },
          {
          ...
          <div
            key={label}
            className="panel rounded-lg p-3 flex items-center gap-3"
          >
            <div
