...
const STANDARD_CATEGORIES = [
  {
    ...
    label: "Bloques Patrón / Calibre",
    prefix: "BLK",
    icon: "",
    color: "#6366F1",
    unit: "mm",
    description: "Bloques Johansson, cintas patrón, calibres de referencia",
    extras: [
      {
        key: "uncertainty_slope",
        label: "Pendiente de Incertidumbre (b)",
        type: "number",
        hint: "b en U = a + b·L [µm/mm]",
        placeholder: "0.001",
      },
    ],
  },
  {
    category: "mass",
    label: "Pesas Patrón OIML",
    prefix: "MAS",
    icon: "⚖️",
    color: "#10B981",
    unit: "g",
    description: "Pesas clase E1/E2/F1/F2/M1/M2/M3 según OIML R111",
    extras: [
      {
        key: "oiml_class",
        label: "Clase OIML",
        type: "select",
        options: ["E1", "E2", "F1", "F2", "M1", "M2", "M3"],
        hint: "Clase de exactitud según OIML R111",
      },
      {
        key: "drift_rate_per_year",
        label: "Tasa de Deriva Anual",
        type: "number",
        hint: "mg/año observado (null = usar estimación OIML)",
        placeholder: "---",
      },
      {
        key: "mass_density",
        ...
        type: "number",
        hint: "kg/m³ para corrección de empuje del aire",
        placeholder: "8000",
      },
    ],
  },
...
function getStatus(expiry: string) {
  ...
  if (days < 90)
    return { label: "Por Vencer", color: COLORS.warning, icon: Clock };
...
function ActionsCell({
  ...
  useEffect(() => {
    const close = (e: MouseEvent) => {
      if (!(e.target as HTMLElement).closest("[data-std-menu]")) setOpen(false);
    };
      ...
      cell: ({ row }) => (
        <div>
          <p
            ...
            style={{ color: "var(--text-main)" }}
          >
            {decodeUnicode(row.original.name ?? "")}
          </p>
          {row.original.brand && (
            <p className="text-[10px]" style={{ color: "var(--text-muted)" }}>
              {decodeUnicode(row.original.brand)}
              {row.original.model
                ? ` · ${decodeUnicode(row.original.model ?? "")}`
                : ""}
            </p>
          )}
        </div>
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span
          ...
          style={{ color: "var(--text-main)" }}
        >
          ±{fmtNum(row.original.uncertainty_u)}{" "}
          <span className="text-[9px]" style={{ color: "var(--text-muted)" }}>
            (k={fmtNum(row.original.k_factor)})
          </span>
        </span>
      ),
    },
...
export default function StandardsPage() {
  ...
  const urlCat = searchParams.get("new");
  const [modalOpen, setModalOpen] = useState(!!urlCat);
  const [editTarget, setEditTarget] = useState<Standard | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Standard | null>(null);
  const [presetCategory, setPresetCategory] = useState<string | null>(urlCat);
  ...
  const columns = useMemo(
    ...
    [setEditTarget, setModalOpen, setDeleteTarget],
  );
...
function StandardModal({
  ...
  const [selectedCat, setSelectedCat] = useState<StdCat | null>(
    editCat ?? presetCat,
  );
  ...
  const autoCode = useMemo(() => {
    ...
    const existing = standards.filter((s) =>
      s.internal_code.startsWith(prefix + "-"),
    );
    const max = existing.reduce((acc, s) => {
      const n = parseInt(s.internal_code.replace(prefix + "-", ""), 10);
  ...
  const {
    ...
    defaultValues: standard
      ? {
          internal_code: standard.internal_code,
          name: standard.name,
  ...
  const onSubmit = async (data: StandardForm) => {
    try {
      ...
      if (isEdit) {
        ...
        toast.success("Patrón de referencia creado");
      }
      qc.invalidateQueries({ queryKey: ["standards"] });
      onClose();
    } catch (err: unknown) {
      ...
      >
        ...
        >
          <div>
            ...
            >
              {isEdit
                ? `Editar — ${standard.internal_code}`
                : selectedCat
                  ? `${selectedCat.icon} Nuevo ${selectedCat.label}`
                  : "Nuevo Patrón de Referencia"}
            </h2>
            <p
            ...
            >
              {isEdit
                ? "Modificar ficha del patrón de referencia"
                : selectedCat
                  ? selectedCat.description
                  : "Selecciona el tipo de magnitud que mide este patrón"}
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              <div className="px-6 py-5 space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    >
                      <input
                        ...
                        placeholder={
                          selectedCat
                            ? `Ej: ${selectedCat.label} de Referencia`
                            : "Nombre"
                        }
                    ...
                    >
                      <input
                        {...register("brand")}
                        placeholder="Mitutoyo, Fluke, OIML…"
                        className="field-input"
                      />
                    </Fld>
                    ...
                    <Fld
                      label="Laboratorio Calibrador"
                      error={errors.calibrated_by_lab?.message}
                    >
                      <input
                        {...register("calibrated_by_lab")}
                        placeholder="INTI, CENLAB, Trescal…"
                        className="field-input"
                      />
                    </Fld>
                  </div>
                ...
                {selectedCat && selectedCat.extras.length > 0 && (
                  <Sec
                    title={`Configuración — ${selectedCat.label}`}
                    hint="Campos específicos para el cálculo de incertidumbre"
                  >
                    <div className="space-y-3">
                      {(selectedCat.extras as readonly StandardExtra[]).map(
                        (ex) => (
                          <Fld
                            key={ex.key}
                            label={ex.label}
                            hint={ex.hint}
                            error={errors[ex.key]?.message}
                          >
                            {ex.type === "select" ? (
                              <select
                                {...register(ex.key)}
              ...
              >
                {!isEdit && (
                  <button
                    ...
                    className="h-8 px-4 rounded text-[11px] font-medium transition-colors hover-bg mr-auto"
                    style={{
                      color: "var(--text-muted)",
                      border: "1px solid var(--border-color)",
                    }}
    ...
    >
      ...
      >
        ...
        <p className="text-[11px] mb-6" style={{ color: "var(--text-muted)" }}>
          ...
          El historial de sesiones de calibración permanecerá intacto.
        </p>
...
function Fld({
  ...
  return (
    <div className={`space-y-1.5 ${className}`}>
      <label
        className="text-[10px] font-semibold uppercase tracking-wider flex items-baseline gap-1.5"
        style={{ color: "var(--text-muted)" }}
      >
        {label}
        {hint && (
