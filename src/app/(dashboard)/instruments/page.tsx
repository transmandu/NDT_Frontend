...
const INSTRUMENT_TYPES = [
  {
    code: "DIM-001",
    label: "Pie de Rey / Vernier",
    prefix: "VER",
    icon: "",
    category: "dimensional",
    unit: "mm",
    unitOptions: ["mm", "cm", "in"],
    color: "#6366F1",
    description: "Calibración dimensional por comparación con bloques patrón",
    extras: [
      {
        key: "functions_exterior",
        label: "Bocas Exteriores",
        type: "checkbox",
        hint: "Habilitar medición exterior",
      },
      {
        key: "functions_interior",
        label: "Bocas Interiores",
        type: "checkbox",
        hint: "Habilitar medición interior",
      },
      {
        key: "functions_depth",
        label: "Sonda de Profundidad",
        type: "checkbox",
        hint: "Habilitar sonda depth",
      },
    ],
  },
  {
    ...
    unitOptions: ["g", "kg", "mg"],
    color: "#10B981",
    description: "Calibración gravimétrica con pesas patrón clase OIML",
    extras: [
      {
        key: "num_points",
        label: "Puntos de calibración",
        type: "number",
        hint: "Cantidad de puntos (ej: 5)",
        placeholder: "5",
        min: 3,
        max: 10,
      },
  ...
  {
    code: "ISO-6789",
    label: "Llave Dinamométrica",
    prefix: "TOR",
    icon: "",
    category: "torque",
    unit: "N·m",
    unitOptions: ["N·m", "kN·m", "lbf·ft", "lbf·in"],
    color: "#F59E0B",
    description: "Calibración según ISO 6789-1:2017 y ISO 6789-2:2017",
    extras: [
      {
        key: "torque_type",
        label: "Tipo de torquímetro",
        type: "select",
        options: [
          "Click (disparador)",
          "Dial (analógico)",
  ...
  {
    code: "ME-003",
    label: "Manómetro de Presión",
    prefix: "MAN",
    icon: "",
    category: "pressure",
    unit: "bar",
    unitOptions: ["bar", "psi", "kPa", "MPa", "Pa"],
    color: "#EF4444",
    description:
      "Calibración por comparación con transductor de presión patrón",
    extras: [
      {
        key: "gauge_type",
        label: "Tipo de manómetro",
        type: "select",
        options: ["Analógico (bourdon)", "Digital", "Diferencial", "Absoluto"],
  ...
  {
    code: "EL-001",
    label: "Multímetro / Voltímetro",
    prefix: "ELM",
    icon: "",
    category: "electrical",
    unit: "V",
    unitOptions: ["V", "mV", "A", "mA", "Ω", "kΩ", "MΩ"],
    color: "#8B5CF6",
    description: "Calibración eléctrica con calibrador de procesos patrón",
    extras: [
      {
        key: "measurement_function",
        label: "Función de medición principal",
        type: "select",
        options: [
          "Tensión DC (VDC)",
          "Tensión AC (VAC)",
        ...
        hint: "Variable eléctrica que se calibrará",
      },
    ],
  },
  {
    ...
    unit: "°C",
    unitOptions: ["°C", "°F", "K", "%HR"],
    color: "#06B6D4",
    description:
      "Calibración de temperatura y humedad relativa en cámara controlada",
    extras: [
      {
        key: "sensor_type",
        ...
        hint: "Principio de medición del sensor",
      },
      {
        key: "measures_humidity",
        label: "Mide Humedad Relativa",
        type: "checkbox",
        hint: "Activar si el equipo también mide %HR",
      },
    ],
  },
] as const;
...
const instrumentSchema = z.object({
  ...
  calibration_interval_months: z.coerce
    .number()
    .int()
    .min(1)
    .max(120)
          ...
          <AnimatePresence>
            {open && pos && (
              ...
              >
                ...
                <Link
                  href={`/calibration/new?instrument_id=${inst.id}`}
      ...
      cell: ({ row }) => {
        ...
        return (
          <span
            ...
            style={{ color: "var(--text-muted)" }}
          >
            {i.range_min ?? 0} – {i.range_max ?? "∞"} {i.unit}
            <br />
            <span className="text-[9px]">
              Res: {i.resolution} {i.unit}
            </span>
          </span>
        );
      },
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const expiryLabel = expiryStr
          ? new Date(expiryStr).toLocaleDateString("es-ES", {
              day: "2-digit",
              month: "2-digit",
              year: "numeric",
            })
        ...
        return (
          <div className="flex flex-col gap-0.5">
            <span
              ...
              style={{ color: expired ? COLORS.danger : "var(--text-main)" }}
            >
              {std.internal_code}
            </span>
            {expired ? (
...
export default function InstrumentsPage() {
  ...
  const columns: ColumnDef<Instrument>[] = useMemo(
    ...
    [standards, setEditTarget, setModalOpen, setDeleteTarget],
  );
...
function InstrumentModal({
  ...
  const editType = isEdit
    ? (INSTRUMENT_TYPES.find((t) =>
        isSameCategory(t.category, instrument.category),
      ) ?? null)
    : null;
  const [selectedType, setSelectedType] = useState<
    (typeof INSTRUMENT_TYPES)[number] | null
  >(editType);

  const autoCode = useMemo(() => {
    if (!selectedType) return "";
    const prefix = selectedType.prefix;
    const existing = instruments.filter((i) =>
      i.internal_code.startsWith(prefix + "-"),
    );
    const maxNum = existing.reduce((acc, i) => {
      const num = parseInt(i.internal_code.replace(prefix + "-", ""), 10);
  ...
  const {
    ...
    defaultValues: instrument
      ? {
          ...
          range_min: instrument.range_min ?? undefined,
          range_max: instrument.range_max ?? undefined,
          location: instrument.location ?? "",
          status:
            (instrument.status as "active" | "inactive" | "in_calibration") ??
            "active",
          emp: instrument.emp ?? undefined,
          calibration_interval_months:
            instrument.calibration_interval_months ?? 12,
  ...
  useEffect(() => {
    ...
    // Lo envolvemos en un setTimeout para salir del flujo síncrono
  ...
  }, [selectedType, autoCode, isEdit, setValue]);
  ...
  const isExpiredDate = (raw: string | null | undefined) =>
    !!raw && raw.slice(0, 10) < today;
  const matchingStandards = selectedType
    ? standards.filter((s) => isSameCategory(s.category, selectedType.category))
    : [];
  const validStandards = matchingStandards.filter(
    (s) => !isExpiredDate(s.expiry_date),
  );
  const expiredStandards = matchingStandards.filter((s) =>
    isExpiredDate(s.expiry_date),
  );
  const selectedStdId = useWatch({ control, name: "factory_standard_id" });
  const selectedStd = selectedStdId
    ? matchingStandards.find((s) => s.id === Number(selectedStdId))
    : null;
  ...
  const humidityStandards = standards.filter((s) =>
    isSameCategory(s.category, "humedad"),
  );
  const humidityStdId = Number(extras.factory_standard_humidity_id) || "";
  const setHumidityStd = (id: string) =>
    setExtras((prev) => ({
      ...prev,
      factory_standard_humidity_id: id || undefined,
    }));
  ...
  const onSubmit = async (data: InstrumentForm) => {
    try {
      const payload = {
        ...data,
        metadata: Object.keys(extras).length > 0 ? extras : null,
      };
      if (isEdit) {
        await api.put(`/instruments/${instrument.id}`, payload);
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              >
                {isEdit
                  ? `✏️ Editar — ${instrument.internal_code}`
                  : selectedType
                    ? `${selectedType.icon} Nuevo ${selectedType.label}`
                    : "➕ Nuevo Instrumento"}
              </h2>
              <p
              ...
              >
                {isEdit
                  ? "Modificar ficha técnica del equipo"
                  : selectedType
                  ...
                  >
                    <div className="flex items-center gap-2 w-full">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p
                          className="text-[9px] font-mono font-medium px-1.5 py-0.5 rounded inline-block mt-0.5"
                          style={{
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              <div className="px-6 py-5 space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    >
                      <div className="relative">
                        ...
                        {!isEdit && (
                          <span
                            className="absolute right-2 top-1/2 -translate-y-1/2 text-[9px] px-1.5 py-0.5 rounded font-medium"
                            style={{
                    ...
                    <Field label="Marca *" error={errors.brand?.message}>
                      <input
                        {...register("brand")}
                        placeholder="Mitutoyo, Fluke, Wika…"
                        className="field-input"
                      />
                    </Field>
                    ...
                    >
                      <input
                        {...register("location")}
                        placeholder="Laboratorio de Metrología"
                        className="field-input"
                      />
                    </Field>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    >
                      {selectedType && selectedType.unitOptions.length > 0 ? (
                        <select {...register("unit")} className="field-input">
                          {selectedType.unitOptions.map((u) => (
                    ...
                    <Field
                      label="Rango Mínimo"
                      error={errors.range_min?.message}
                    >
                      <div className="relative">
                        <input
                          {...register("range_min")}
                    ...
                    <Field
                      label="Rango Máximo"
                      error={errors.range_max?.message}
                    >
                      <div className="relative">
                        <input
                          {...register("range_max")}
                    ...
                    >
                      <div className="relative">
                        <input
                          {...register("calibration_interval_months")}
                          type="number"
                          step="1"
                          min="1"
                          max="120"
                ...
                {selectedType && selectedType.extras.length > 0 && (
                  <Section
                    title={`Configuración — ${selectedType.label}`}
                    hint="Campos específicos para el procedimiento de calibración"
                  >
                    <div className="space-y-3">
                      {(selectedType.extras as readonly ExtraField[]).map(
                        (extra) => (
                          <div key={extra.key}>
                            <label
                            ...
                            {extra.type === "checkbox" ? (
                              ...
                              <select
                                className="field-input"
                                value={
                                  typeof extras[extra.key] === "string"
                                    ? (extras[extra.key] as string)
                                    : ""
                                }
                                onChange={(e) =>
                                  setExtras((prev) => ({
                              ...
                              <input
                                ...
                                value={
                                  typeof extras[extra.key] === "string" ||
                                  typeof extras[extra.key] === "number"
                ...
                <Section
                  title="Patrón de Referencia de Fábrica"
                  hint="(opcional)"
                >
                  {matchingStandards.length === 0 && selectedType ? (
                    <p
                      ...
                      style={{ color: "var(--text-muted)" }}
                    >
                      No hay patrones para{" "}
                      <strong>{selectedType.category}</strong>.
                    </p>
                  ) : (
                    <>
                      <Field
                        label={
                          isThermohygrometer
                            ? "Patrón Temperatura (principal)"
                            : "Patrón asociado"
                        }
                        error={errors.factory_standard_id?.message}
                      >
                        <select
                          {...register("factory_standard_id")}
                          className="field-input"
                        >
                          <option value="">— Ninguno / No aplica —</option>
                          {validStandards.map((s) => (
                            <option key={s.id} value={s.id}>
                              {s.internal_code} — {s.name} · Vence:{" "}
                              {fmtExpiry(s.expiry_date)} (U={s.uncertainty_u},
                              k={s.k_factor})
                            </option>
                          ))}
                          {expiredStandards.length > 0 && (
                            <optgroup label="── VENCIDOS (no recomendado) ──">
                              {expiredStandards.map((s) => (
                                <option key={s.id} value={s.id}>
                                  ⚠ {s.internal_code} — {s.name} · VENCIDO:{" "}
                      ...
                      {selectedIsExpired && (
                        <div
                          className="mt-2 flex items-start gap-2 rounded px-3 py-2 text-[11px] font-medium"
                          style={{
                        ...
                        >
                          ...
                          />
                          <span>
                            El patrón seleccionado está{" "}
                            <strong>
                              vencido desde{" "}
                              {fmtExpiry(selectedStd?.expiry_date)}
                            </strong>
                            . Asigna uno vigente antes de usarlo en una
                            calibración.
                          </span>
                        </div>
                      )}
                  ...
                  {isThermohygrometer && (
                    ...
                    >
                      <Field label="Patrón Humedad Relativa (secundario)">
                        {humidityStandards.length === 0 ? (
                          <p
                            ...
                            style={{ color: "var(--text-muted)" }}
                          >
                            No hay patrones de humedad registrados.
                          </p>
                        ) : (
                          <select
                            value={humidityStdId}
                      ...
                      <p
                        ...
                        style={{ color: "var(--text-muted)" }}
                      >
                        Patrón independiente para la magnitud Humedad Relativa
                        (std[1]). ISO 17025 §6.4.
                      </p>
                    </div>
                  )}
    ...
    >
      ...
      >
        ...
        <p className="text-[11px] mb-6" style={{ color: "var(--text-muted)" }}>
          Se realizará un <strong>soft delete</strong> de{" "}
          <strong>{name}</strong>.<br />
          El historial de calibraciones permanecerá intacto.
        </p>
