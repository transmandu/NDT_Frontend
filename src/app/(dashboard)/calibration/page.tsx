...
export default function CalibrationPage() {
  ...
  const handleDeleteDraft = async (id: number) => {
    if (
      !window.confirm(
        `Eliminar borrador CS-${id}? Esta accion no se puede deshacer.`,
      )
    )
      return;
    setDeletingId(id);
  ...
  if (reviewingId) {
    return (
      <CalibrationReview id={reviewingId} onBack={() => setReviewingId(null)} />
    );
  }
  ...
  return (
    <div className="space-y-4 w-full animate-fadeIn">
      ...
      <div
        id="tour-center-tabs"
        className="flex flex-wrap mb-4 gap-1"
      ...
      >
        {(
          ...
          <button
            ...
            style={{
              color: activeTab === key ? COLORS.primary : "var(--text-muted)",
          ...
          <table className="w-full text-left text-xs min-w-[600px]">
            <thead>
              <tr>
                ...
                <th className="px-4 py-2 th-theme text-[11px]">
                  Fecha Medición
                </th>
            ...
            >
              {isLoading ? (
                Array.from({ length: 4 }).map((_, i) => (
                  <tr
                    key={`skel-p-${i}`}
                    className="td-theme border-b border-(--border-color)"
                  >
                ...
                pending.map((s) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      {s.instrument?.name || `Inst #${s.instrument_id}`}
                    </td>
                    <td
                      className="px-4 py-3"
                      style={{ color: "var(--text-muted)" }}
          ...
          <table className="w-full text-left text-xs min-w-[600px]">
            ...
            >
              {isLoading ? (
                ...
                drafts.map((s) => {
                  const resumable =
                    s.created_at && isDraftResumable(s.created_at);
                  return (
                    <tr
                      key={s.id}
                      className="td-theme hover-bg transition-colors"
                    >
                      ...
                      <td className="px-4 py-3 text-right flex items-center justify-end gap-2">
                        ...
                        <button
                          onClick={() => router.push(`/calibration/${s.id}`)}
            ...
            >
              {loadingCerts ? (
                  ...
                  <td colSpan={7} className="px-4 py-10 text-center">
                    ...
                    >
                      ...
                      <span className="text-[10px] opacity-70">
                        Los certificados aparecen aquí al aprobar una sesión de
                        calibración.
                      </span>
                    </div>
                  </td>
                ...
                certificates.map((cert) => (
                  ...
                  >
                    <td
                      className="px-4 py-2.5 font-mono font-bold text-[11px]"
                    ...
                    <td
                      className="px-4 py-2.5 hidden sm:table-cell text-[11px]"
                      style={{ color: "var(--text-muted)" }}
                    >
