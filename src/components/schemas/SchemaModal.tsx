...
function validateUiSchema(raw: string): { ok: boolean; message: string } {
  try {
    ...
    if (!parsed.procedure_code)
      return { ok: false, message: 'Falta "procedure_code"' };
    if (!parsed.procedure_name)
      return { ok: false, message: 'Falta "procedure_name"' };
    if (!Array.isArray(parsed.metadata_requirements))
      return { ok: false, message: 'Falta array "metadata_requirements"' };
    if (!Array.isArray(parsed.grids) || parsed.grids.length === 0)
      return {
        ok: false,
        message: 'Falta array "grids" con al menos 1 elemento',
      };
    for (const g of parsed.grids) {
      ...
      if (!Array.isArray(g.columns) || g.columns.length === 0)
        return { ok: false, message: `Grid "${g.id}" sin "columns"` };
    }
    return {
      ok: true,
      message: `Válido · ${parsed.grids.length} grid(s) · ${parsed.metadata_requirements.length} campo(s) de metadata`,
    };
  } catch (err) {
    return {
      ok: false,
...
function validateOptionalJson(raw: string): { ok: boolean; message: string } {
  if (!raw || raw.trim() === "" || raw.trim() === "null")
    return { ok: true, message: "Vacío (se guardará null)" };
  try {
    JSON.parse(raw);
    return { ok: true, message: "JSON válido" };
  } catch (err: unknown) {
    return {
      ok: false,
      message: err instanceof Error ? err.message : "JSON inválido",
...
function JsonEditor({
  ...
  validator: (v: string) => { ok: boolean; message: string };
  required?: boolean;
}) {
...
export default function SchemaModal({
  ...
  const {
    ...
    defaultValues: isEdit
      ? {
          ...
          math_config_raw: schema.math_config
            ? JSON.stringify(schema.math_config, null, 2)
            : "",
          validation_rules_raw: schema.validation_rules
  ...
  // Dentro del componente, después del useForm:
  const uiRaw = useWatch({ control, name: "ui_schema_raw" }) ?? "";
  const mathRaw = useWatch({ control, name: "math_config_raw" }) ?? "";
  const valRaw = useWatch({ control, name: "validation_rules_raw" }) ?? "";
  const is_active = useWatch({ control, name: "is_active" }) ?? false;
  ...
  const mutation = useMutation({
    ...
    onError: (err: unknown) => {
      if (isAxiosError(err)) {
        const msgs = err.response?.data?.errors as
        ...
        toast.error(
          err.response?.data?.message || "Error al guardar el esquema.",
        );
        return;
      }
      ...
      >
        ...
        >
          <div>
            ...
            >
              {isEdit
                ? `Editar Esquema — ${schema.code}`
                : "Nuevo Esquema de Procedimiento"}
            </h2>
            <p
        ...
        >
          {(
            [
              ["basic", Settings2, "Información General"],
              ["json", FileCode2, "Configuración JSON"],
            ] as const
          ).map(([key, Icon, label]) => (
            <button
              key={key}
              ...
              style={{
                ...
                borderBottom:
                  tab === key ? `2px solid ${ACCENT}` : "2px solid transparent",
              }}
            >
              <Icon size={13} />
              {label}
        ...
        >
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field
                    ...
                    hint="Mayúsculas y guiones. Ej: DIM-001"
                  >
                    <input
                      {...register("code")}
                      disabled={isEdit}
                ...
                >
                  <input
                    ...
                    placeholder="Calibración Dimensional — Pie de Rey"
                  />
                </Field>
                ...
                {/* Info card */}
                <div
                  className="p-3 rounded-lg text-[10px] leading-relaxed space-y-1"
                  style={{
                ...
                >
                  ...
                  <p>
                    1. Completa los campos de esta pestaña para identificar el
                    procedimiento.
                  </p>
                  <p>
                    2. Ve a <strong>Configuración JSON</strong> para definir el{" "}
                    <code>ui_schema</code> (grids de medición) y el{" "}
                    <code>math_config</code> (parámetros del motor de cálculo).
                  </p>
                  <p>
                    3. El código debe coincidir con el{" "}
                    <code>procedure_code</code> dentro del{" "}
                    <code>ui_schema</code>.
                  </p>
                </div>
              </div>
          ...
          >
            <p className="text-[10px]" style={{ color: "var(--text-muted)" }}>
              {errors.ui_schema_raw && (
                <span className="text-red-400">
                  Verifica el UI Schema en la pestaña JSON
                </span>
              )}
            </p>
