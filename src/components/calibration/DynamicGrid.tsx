...
function stddev(vals: number[]): number {
  ...
  const m = avg(vals);
  return Math.sqrt(
    vals.reduce((a, b) => a + (b - m) ** 2, 0) / (vals.length - 1),
  );
}
...
function computeValue(
  ...
  const mainCol = gridColumns.find(
    (c) =>
      c.type === "number_array" &&
      (c.key === "readings" || c.key === "reading"),
  );
  const instrCol = gridColumns.find(
    (c) => c.type === "number_array" && c.key === "instrument_reading",
  );
  const stdCol = gridColumns.find(
    (c) => c.type === "number_array" && c.key === "standard_reading",
  );
  ...
  if (key === "deviation_um") {
    ...
    const readCol = gridColumns.find(
      (c) => c.type === "number_array" && c.key === "readings",
    );
...
function TransposedMatrix({
  ...
  const headerCols = columns.filter(
    (c) => c.type !== "number_array" && !c.computed,
  );
  ...
  useEffect(() => {
    ...
    if (first) {
      ...
      if (reps.length > 0 && reps.length !== repCount) {
        const timer = setTimeout(() => setRepCount(reps.length), 0);
        return () => clearTimeout(timer);
      }
    }
  }, [data, arrayCols, repCount]);

  const getCell = useCallback(
    (ptIdx: number, key: string) => data[ptIdx]?.[key] || "",
    [data],
  );
  const getReps = useCallback(
    (ptIdx: number, key: string): string[] => {
  ...
  const addRepetition = () => {
    ...
    for (const k of Object.keys(data).map(Number)) {
      const row = { ...data[k] };
      for (const ac of arrayCols)
        row[ac.key] = [...parseArr(row[ac.key] || ""), ""].join(",");
      nd[k] = row;
    }
    setRepCount((c) => c + 1);
    onChange(grid.id, nd);
  ...
  const removeRepetition = (repIdx: number) => {
    ...
    for (const k of Object.keys(data).map(Number)) {
      const row = { ...data[k] };
      for (const ac of arrayCols)
        row[ac.key] = parseArr(row[ac.key] || "")
          .filter((_, i) => i !== repIdx)
          .join(",");
      nd[k] = row;
    }
  ...
  const addPoint = () => {
    ...
    for (const ac of arrayCols)
      row[ac.key] = Array(repCount).fill("").join(",");
  ...
  const pointStats = useMemo(() => {
    ...
    return r;
  }, [arrayCols, pts, getCell]);

  const getComputed = (ptIdx: number, col: GridColumn) =>
    computeValue(col, ptIdx, data[ptIdx] || {}, data, columns);
  const colW = 68;
  ...
  return (
    <div className="flex flex-col gap-2 w-full">
        ...
        {repCount < maxReps && (
          <button
            ...
            style={{
              ...
              color: ORANGE,
              backgroundColor: "rgba(255,165,38,0.08)",
            }}
      ...
      <div
        className="rounded-md overflow-x-auto w-full"
        style={{ border: "1px solid var(--border-color)" }}
      >
        <table
          className="w-full text-xs"
          style={{ minWidth: `${170 + pts.length * colW}px` }}
        >
          <thead>
            <tr
              style={{
            ...
            >
              <th
                className="px-3 py-1.5 text-left text-[11px] align-top"
                style={{
              ...
              >
                <div className="flex flex-col gap-0.5 items-start pt-1 pb-0.5">
                  {headerCols.map((hc) => (
                    <div
                      key={hc.key}
                      className="h-6 flex items-center justify-start text-[10px] uppercase font-medium tracking-wide"
                      style={{ color: "var(--text-secondary)" }}
                    >
                      {hc.label}{" "}
                      {hc.unit && !hc.label.includes(hc.unit)
                        ? `[${hc.unit}]`
                        : ""}
                      {hc.required && (
                        <span className="text-red-500 text-[8px] ml-1">●</span>
                      )}
                    </div>
                  ))}
                </div>
              ...
              {pts.map((ptIdx, idx) => (
                <th
                  key={ptIdx}
                  className="py-1.5 px-1 text-center relative group align-top"
                  style={{
                    borderRight:
                      idx < pts.length - 1
                  ...
                  <div className="flex flex-col gap-0.5 items-center pt-1 pb-0.5">
                    {headerCols.map((hc) => (
                      <input
                        key={hc.key}
                        type="text"
                        inputMode={hc.type === "number" ? "decimal" : "text"}
                        value={getCell(ptIdx, hc.key)}
                        onChange={(e) =>
                          updateCell(ptIdx, hc.key, e.target.value)
                        }
                        ...
                        style={{
                          ...
                          border: "1px dashed rgba(255,165,38,0.4)",
                        }}
                      />
                    ))}
                  </div>
          ...
          <tbody>
                ...
                {multiArray && (
                  <tr style={{ backgroundColor: "rgba(255,165,38,0.06)" }}>
                    <td
                      ...
                      className="px-3 py-1 text-[10px] font-bold tracking-wide"
                      style={{
                        borderTop:
                          acIdx > 0
                    ...
                    >
                      ▸ {ac.label}
                      {ac.unit && !ac.label.includes(ac.unit) && (
                        <span className="ml-1 font-normal opacity-60 text-[9px]">
                          ({ac.unit})
                        </span>
                      )}
                    </td>
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        {acIdx === 0 && repCount > minReps && (
                          <button
                            onClick={() => removeRepetition(repIdx)}
                    ...
                    {pts.map((ptIdx, colIdx) => (
                      ...
                      >
                        <input
                          ...
                          onChange={(e) =>
                            updateRep(ptIdx, ac.key, repIdx, e.target.value)
                          }
                          placeholder="—"
            ...
            {computedCols.map((col) => (
              <tr
                key={col.key}
                style={{ backgroundColor: "var(--bg-result-row)" }}
              >
                <td
                ...
                >
                  {col.label}
                  {col.unit && !col.label.includes(col.unit) && (
                ...
                {pts.map((ptIdx, idx) => {
                  ...
                  return (
                    <td
                      key={ptIdx}
                      className="px-2 py-2 text-center font-mono text-[12px] font-bold"
                      style={{
                        borderRight:
                          idx < pts.length - 1
                            ? "1px solid var(--border-color)"
              ...
              <>
                <tr style={{ backgroundColor: "var(--bg-result-row)" }}>
                  ...
                  >
                    Media (x̄)
                    {multiArray && (
                      <span className="font-normal opacity-50 text-[9px] ml-1">
                        — {arrayCols[0].label}
                      </span>
                    )}
                  </td>
...
function StandardGrid({
    ...
    for (let i = 0; i < rowCount; i++) {
      ...
      for (const col of columns) {
        if (col.type === "number_array")
          init[i][col.key] = Array(minIter).fill("").join(",");
      }
    }
    onChange(grid.id, init);
  }, [rowCount, data, columns, grid.id, minIter, onChange, rowLabels]);
  ...
  const addRow = () => {
    ...
    for (const col of columns) {
      if (col.type === "number_array")
        newRow[col.key] = Array(minIter).fill("").join(",");
    }
  ...
  const removeRow = (rowIdx: number) => {
    ...
    let ni = 0;
    for (const k of keys) {
      if (k !== rowIdx) nd[ni++] = data[k];
    }
    onChange(grid.id, nd);
  };

  const getReps = (rowIdx: number, colKey: string) =>
    parseArr(data[rowIdx]?.[colKey] || "");
  const updateRep = (
    rowIdx: number,
    colKey: string,
    repIdx: number,
  ...
  const numericCols = columns.filter(
    (c) =>
      (c.type === "number" || c.type === "number_array") &&
      c.key !== "reading_order" &&
      c.key !== "nominal_value",
  );
  ...
  const computeStats = (colKey: string) => {
    ...
    for (const rIdx of Object.keys(data).map(Number)) {
      const row = data[rIdx];
      let val: string;
      if (col.computed) val = computeValue(col, rIdx, row, data, columns);
      else val = row[colKey] || "";
      if (col.type === "number_array")
        parseArr(val).forEach((v) => {
    ...
    return {
      mean: fmt(avg(vals), 4),
      std: vals.length > 1 ? fmt(stddev(vals), 4) : "0.0000",
    };
  };
    ...
    <div className="flex flex-col gap-2 w-full">
      ...
      >
        <table
          ...
          style={{ minWidth: `${Math.max(columns.length * 90, 500)}px` }}
        >
          <thead>
            <tr>
              <th
                className="px-2 py-2 th-theme text-center w-9 text-[10px]"
                style={{
                  borderBottom: "1px solid var(--border-color)",
                  borderRight: "1px solid var(--border-color)",
                }}
              ...
              {columns.map((col) => (
                <th
                  key={col.key}
                  className="px-1.5 py-2 th-theme text-[9px] uppercase tracking-wider font-semibold leading-tight"
                  style={{
                    borderBottom: "1px solid var(--border-color)",
                    borderRight: "1px solid var(--border-color)",
                  }}
                >
                  <div className="flex flex-col gap-0.5">
                    ...
                    {col.unit && !col.label.includes(col.unit) && (
                      <span className="text-[9px] font-mono font-normal opacity-50">
                        {col.unit}
                      </span>
                    )}
                  </div>
                        ...
                        return (
                          <td
                            ...
                            style={{
                              borderRight: "1px solid var(--border-color)",
                              borderBottom: "1px solid var(--border-color)",
                              backgroundColor: "var(--bg-result-row)",
                            }}
                          >
                        ...
                        return (
                          ...
                          >
                            <select
                              ...
                              onChange={(e) =>
                                update(rIdx, col.key, e.target.value)
                              }
                              ...
                              {reps.map((rep, repIdx) => (
                                ...
                                >
                                  ...
                                  {reps.length > curMin && (
                                    <button
                                      onClick={() =>
                                        removeRep(rIdx, col.key, repIdx)
                                      }
                                      className="absolute -top-1.5 -right-1.5 w-3.5 h-3.5 rounded-full bg-red-500 text-white items-center justify-center text-[8px] hidden group-hover/rep:flex shadow-sm"
                                    >
                                      ✕
                                    </button>
                                  )}
                    ...
                    <tr style={{ backgroundColor: "var(--bg-result-row)" }}>
                      ...
                      {columns.map((col) => (
                        <td
                          ...
                          style={{
                            color: "var(--text-result-val)",
                            borderRight: "1px solid var(--border-color)",
                            borderTop: fb,
                          }}
                        >
                          {numericCols.some((nc) => nc.key === col.key)
                            ? computeStats(col.key).mean
                            : ""}
                        </td>
                      ))}
...
function HorizontalIterationsGrid({
  grid,
  data,
  onChange,
  validationErrors,
  ...
  useEffect(() => {
    if (!editableCol || Object.keys(data).length > 0) return;
    const init: GridData = {};
    for (let i = 0; i < rowCount; i++) {
    ...
    onChange(grid.id, init);
  }, [rowCount, data, columns, editableCol, grid.id, onChange]);
  ...
  const updateCell = (rIdx: number, val: string) =>
    onChange(grid.id, {
      ...data,
      [rIdx]: { ...(data[rIdx] || {}), [editableCol.key]: val },
    });
  ...
  return (
    ...
    >
      <table
        className="w-full text-xs"
        style={{ minWidth: `${100 + rowKeys.length * 60}px` }}
      >
        <thead>
          <tr
            style={{
          ...
          >
            ...
            >
              {editableCol.label}{" "}
              {editableCol.unit &&
                !editableCol.label.includes(editableCol.unit) && (
        ...
        <tbody>
          <tr style={{ backgroundColor: "var(--bg-panel)" }}>
            ...
            {rowKeys.map((rIdx) => {
              ...
              const hasError = validationErrors?.has(
                `${rIdx}-${editableCol.key}`,
              );
              return (
                <td
                  key={rIdx}
                ...
                >
                  <input
                    ...
                    inputMode="decimal"
                    value={val}
                    onChange={(e) => updateCell(rIdx, e.target.value)}
                    placeholder="—"
...
/* ─── Main Export ────────────────────────────────────────── */
export default React.memo(function DynamicGrid({
  grid,
  gridIndex,
  data,
  onChange,
  ...
  return (
    <div className="w-full">
      <div className="mb-3 flex items-start gap-3">
        ...
        {isTransposed && (
          <span
            className="text-[9px] px-2 py-0.5 rounded font-mono shrink-0 mt-0.5"
            style={{
