...
const distLabel = (d: string) =>
  ({ normal: "Normal", rectangular: "Rectangular", triangular: "Triangular" })[
    d
  ] ?? d;

const funcLabel = (k?: string) =>
  ({ exterior: "Bocas Ext.", interior: "Bocas Int.", depth: "Profundidad" })[
    k ?? ""
  ] ??
  k ??
  "";
...
function TypeBadge({ type }: { type: "A" | "B" }) {
  ...
  return (
    <span
      className="inline-flex items-center justify-center text-[9px] font-bold rounded px-1.5 py-0.5 uppercase"
      style={{
        backgroundColor: isA
          ? "rgba(59,130,246,0.15)"
          : "rgba(255,165,38,0.15)",
...
function Step({
  ...
  return (
    ...
    >
      ...
      />
      <div
        className="absolute left-0 top-0 w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold text-white"
        ...
        <thead>
          <tr style={{ backgroundColor: "var(--bg-app)" }}>
            {[
              ...
              <th
                key={i}
                className={`px-2 py-2 text-[10px] font-semibold ${i === 0 ? "text-left" : "text-center"}`}
                style={{ ...thStyle, ...(i === 6 ? { color: ORANGE } : {}) }}
              >
                {h}
              </th>
            ))}
        ...
        <tbody>
          {src.map((s, i) => (
            ...
            >
              ...
              <td className="px-2 py-2 text-center font-mono text-[10px]">
                {f(s.sensitivity_coefficient, 2)}
              </td>
              <td
                className="px-2 py-2 text-center font-mono text-[11px] font-bold"
                style={{ color: ORANGE }}
              >
                {f(s.standard_uncertainty)}
              </td>
              <td
    ...
    >
      <div>
        ...
        <p
          ...
          style={{ color: "var(--text-muted)" }}
        >
          El presupuesto es una propiedad del instrumento — idéntico para todos
          los puntos nominales. Se muestra una sola vez con los valores reales
          sustituidos.
        </p>
      </div>
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(135deg,rgba(255,165,38,0.08),rgba(255,165,38,0.02))",
      ...
      >
        {readings.length > 0 && (
          <Step number={sn++} title="Lecturas Registradas">
            <MathBlock
              tex={`x_i = \\{${readingsStr}\\} \\quad [\\text{${texUnit}}]`}
        ...
        {mean !== undefined && (
          <Step number={sn++} title="Media Aritmética">
            <MathBlock
              ...
              label="Resultado"
              style={{
                borderColor: "rgba(255,165,38,0.3)",
                backgroundColor: "rgba(255,165,38,0.04)",
              }}
            />
          </Step>
        )}
        ...
        {point.std_deviation_um !== undefined && (
          <Step number={sn++} title="Desviación Estándar (Tipo A)">
            <MathBlock
              tex={`s_j = ${f(point.std_deviation_um)} \\;\\mu\\text{m}`}
              label="Repetibilidad del punto"
            />
            {point.sj_used_um !== undefined && point.sj_source && (
              ...
              >
                <strong style={{ color: ORANGE }}>s_j utilizado:</strong>{" "}
                {f(point.sj_used_um)} µm — <em>{point.sj_source}</em>
              </div>
            )}
          </Step>
        )}
...
/* ─── Main Export ───────────────────────────────────────────── */
export default function AuditMathBreakdown({
  results,
  instrumentUnit,
}: {
  results: PointResult[];
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            onClick={() => setExpandedPoints(new Set(results.map((_, i) => i)))}
          ...
          <button
            onClick={() => setExpandedPoints(new Set())}
            className="text-[10px] px-2 py-1 rounded transition-colors hover-bg"
            style={{
              border: "1px solid var(--border-color)",
              color: "var(--text-muted)",
      ...
      {results.map((point, idx) => {
        ...
        const U_display =
          point.expanded_uncertainty_um !== undefined
          ...
          <div key={idx}>
            ...
            >
              <div className="flex items-center gap-3">
                <Sigma
                  size={16}
                  style={{ color: isOpen ? ORANGE : "var(--text-muted)" }}
                />
                <span
