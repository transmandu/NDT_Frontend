              ...
              >
                ...
                {table
                  ...
                  .map((col) => (
                    <label
                      key={col.id}
                      className="flex items-center gap-2 px-3 py-1.5 text-[11px] cursor-pointer hover-bg"
                      style={{ color: "var(--text-main)" }}
                    >
                      <input
                        type="checkbox"
...
function FilterPopover<TData, TValue>({
  ...
  const faceted: Map<string, number> =
        ...
        >
          ...
          {options.map((opt) => (
            <button
              key={opt}
              onClick={() => {
                column.setFilterValue(opt === currentValue ? undefined : opt);
                setOpen(false);
              }}
...
function getPaginationRange(
  ...
  const range: (number | "…")[] = [];
  [0, current - 1, current, current + 1, pageCount - 1].forEach((p) => {
    if (p >= 0 && p < pageCount) range.push(p);
  });
  const unique = [...new Set(range)].sort(
    (a, b) => (a as number) - (b as number),
  );
  const result: (number | "…")[] = [];
  unique.forEach((p, i) => {
    if (i > 0 && (p as number) - (unique[i - 1] as number) > 1)
      result.push("…");
    result.push(p);
  });
  return result;
